...
- Реализация: Кастомные обработчики исключений в `app/core/exceptions.py` модифицированы для генерации ответов в формате RFC 7807.
- Тестирование:
    - Автоматический тест `tests/test_errors.py::test_not_found_error_conforms_to_rfc7807` успешно проходит, проверяя, что ошибка 404 содержит все обязательные поля (`type`, `title`, `status`, `detail`, `correlation_id`).
    - Автоматический тест `tests/test_errors.py::test_validation_error_conforms_to_rfc7807` успешно проходит, проверяя структуру ошибки 422.
