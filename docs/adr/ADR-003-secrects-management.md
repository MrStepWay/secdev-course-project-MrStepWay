...
- Реализация: Код приложения (`app/core/config.py`) остаётся неизменным и продолжает читать конфигурацию из переменных окружения. Никакие секреты не добавлены в код или файлы конфигурации в репозитории. Добавлена интеграция с Vault.
- Документация: В `README.md` проекта добавлен раздел, описывающий использование файла `.env` для локальной разработки и явно указывающий, что этот метод не должен использоваться в production.
- **CI/CD:** CI-пайплайн сконфигурирован для безопасной передачи секрета `DATABASE_URL` в виде переменной окружения на шаге запуска тестов. Сборка и тесты успешно проходят, подтверждая, что приложение корректно стартует с конфигурацией, переданной через окружение.
